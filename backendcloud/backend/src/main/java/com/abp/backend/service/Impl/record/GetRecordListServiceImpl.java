package com.abp.backend.service.Impl.record;
...
import com.abp.backend.service.record.GetRecordListService;
...
public class GetRecordListServiceImpl implements GetRecordListService {
    ...
    public JSONObject getList(Integer page) {
        ...
        queryWrapper.orderByDesc("id");                     // 使用 id 排序防止两个对局时间相同
        ...
        for (Record record : records) {
            ...
            if ("A".equals(record.getWinner()))
                result = userA.getUsername();
            else if ("B".equals(record.getWinner()))
                result = userB.getUsername();
